...
const COL = {
  ...
  STATUS:     13,  // N
  ANALISTA:   14,  // O
  // Q (Data execução) e R (Hora execução) não são gravadas por este script — são preenchidas
  // por outro mecanismo da planilha, não pela automação.
  AGENTE:     18,  // S
  OBSERVACAO: 19,  // T
};
...
async function registrarFalha(rowNum, sol, categoriaAgente, detalhe = '') {
  ...
  // só as colunas de diagnóstico (Analista, categoria do Agente e Observação). Data/Hora de
  // execução (Q/R) não são gravadas por este script.
  return atualizarPlanilha(rowNum, [
    [COL.ANALISTA,   'AGENTE DE IA'],
